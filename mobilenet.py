        ...
        if inp == hidden_dim:
            self.conv = nn.Sequential(
                ...
                nn.Conv2d(hidden_dim, hidden_dim, kernel_size, stride,
                          (kernel_size - 1) // 2, groups=hidden_dim,
                          bias=False),
...
class MobileNetV3(nn.Module):
    def __init__(self, cfgs, mode, num_classes=66, width_mult=1.):
        ...
        self.lin_yaw = nn.Linear(output_channel, num_classes)
        self.lin_pitch = nn.Linear(output_channel, num_classes)
        self.lin_roll = nn.Linear(output_channel, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.middle(x)
        yaw = self.lin_yaw(x)
        pitch = self.lin_pitch(x)
        roll = self.lin_roll(x)
